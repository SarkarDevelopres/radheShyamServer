...
function checkWinner(playerA, playerB) {
    const weightA = Object.entries(winWeightage).find(([k]) => playerA[k])?.[1] || 1;
    const weightB = Object.entries(winWeightage).find(([k]) => playerB[k])?.[1] || 1;

    if (weightA !== weightB) {
        if (weightA > weightB) return "WINNER_PLAYERA";
        if (weightB > weightA) return "WINNER_PLAYERB";
    }

    // Same category → compare highest rank
    const aHigh = RANK_VAL[playerA.HighestRank];
    const bHigh = RANK_VAL[playerB.HighestRank];
...
const marketKeys = [
    ...
    "PlayerA_JandQ", "PlayerB_JandQ", "Winner_PlayerA", "Winner_PlayerB"
];
...
function initTeenpattiT20(io, tableId) {
...
module.exports = { initTeenpattiT20 }
