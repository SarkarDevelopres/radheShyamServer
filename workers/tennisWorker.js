...
async function fetchCompletedTennisIds() {
    ...
    // Walk Over Cancelled
    return result
        .filter(it => /finished|retired|walk\s*over|cancelled/i.test(it.event_status))
        .map(it => ({
            matchId: String(it.event_key),
...
async function settleSportMatches(sport, completed) {
    ...
    try {
        ...
        console.log('[settle] completed matches array:', completed);
    } catch (e) {
...
async function runSettlement() {
    try {
        let completed = await withTimeout(fetchCompletedTennisIds(), 60_000, 'completed:tennis');
        await settleSportMatches('tennis', completed);
        console.log('[runSettlement] completed IDs:', completed.length);
    } catch (e) {
        console.error('[runSettlement] tennis error:', e);
    }
}
